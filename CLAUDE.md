# Claude Code Project Instructions — async-trait-kotlin

## Project Overview

This is **async-trait-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`async-trait`](https://crates.io/crates/async-trait).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/async-trait/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/async-trait>
