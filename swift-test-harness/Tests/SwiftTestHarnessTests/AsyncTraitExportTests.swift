...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AsyncTrait.swiftmodule/`
...
//      `import AsyncTrait` above would fail at compile time.
//
//   2. The static archive `libAsyncTrait.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AsyncTrait" }` and
//      `flattenPackage = "io.github.kotlinmania.asynctrait"` configuration in
...
//      via the `AsyncTraitLibrary` product.
//
...
final class AsyncTraitExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AsyncTrait swift module imported cleanly")
    }
}
